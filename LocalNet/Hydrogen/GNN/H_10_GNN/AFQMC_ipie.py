...
if rank == 0:
    ...
    # Extract raw energies, dropping the first block (equilibration step)
    raw_block_energies = qmc_data["ETotal"][1:]
    
    df_ac = reblock_by_autocorr(raw_block_energies, verbose=0)
    ...
    metrics = {
        ...
        "results": {
            ...
            "raw_block_energies": raw_block_energies.tolist() # Saved for error bar diff calculations
        },
        "local_energy_proxy": {
    ...
    print(f"{'='*50}")
    # Print it without the raw energies to keep the stdout clean
    print_metrics = metrics.copy()
    print_metrics["results"] = {k: v for k, v in metrics["results"].items() if k != "raw_block_energies"}
