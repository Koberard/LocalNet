...
def build_gnn_model(dist_matrix, adj_matrix, dynamic_r_max):
...
def create_ml_local_energy_patch(ml_model, y_scaler, node_scaler, edge_scaler, Z_array_scaled, P_hf_ref, E_hf_ref, S_sqrt, h_core_dyn, C_a, C_b, aoslices, use_gpu, n_atoms):
    ...
    def local_energy_single_det_uhf(system, hamiltonian, walkers, trial):
        ...
        # Contract full basis set down to atomic nodes/edges
        ...
        # Build Nodes with Z charges
        X_nodes_raw = np.real(np.diagonal(delta_P_atom, axis1=1, axis2=2))[..., None]
        Z_features = np.tile(Z_array_scaled[None, :, None], (nwalkers, 1, 1))
        X_nodes = np.concatenate([X_nodes_raw, Z_features], axis=-1)
        
        # Scale Nodes
        orig_node_shape = X_nodes.shape
        X_nodes = node_scaler.transform(X_nodes.reshape(-1, 2)).reshape(orig_node_shape).astype(np.float32)

        # Build Edges
        X_edges = np.stack([np.real(delta_P_atom), np.imag(delta_P_atom)], axis=-1)
        
        # Scale Edges
        orig_edge_shape = X_edges.shape
        X_edges = edge_scaler.transform(X_edges.reshape(-1, 2)).reshape(orig_edge_shape).astype(np.float32)
...
if rank == 0:
    ...
    # Compute scaled Z charges
    Z_array = np.array([mol.atom_charge(i) for i in range(mol.natm)], dtype=np.float32)
    Z_array_scaled = Z_array / 6.0

    print(">>> Rebuilding GNN Architecture...")
    ml_model = build_gnn_model(dist_matrix, adj_matrix, DYNAMIC_R_MAX)
    ml_model.load_weights(WEIGHTS_PATH)
    
    dummy_nodes = np.zeros((1, N_ATOMS, 2), dtype=np.float32)
    dummy_edges = np.zeros((1, N_ATOMS, N_ATOMS, 2), dtype=np.float32)
    ml_model([dummy_nodes, dummy_edges])
else: 
    E_HF = C_a = C_b = S_sqrt = h_core = ml_model = y_scaler = node_scaler = edge_scaler = Z_array_scaled = P_hf_ref = aoslices = mol = mf = None
...
node_scaler = comm.comm.bcast(node_scaler, root=0)
edge_scaler = comm.comm.bcast(edge_scaler, root=0)
Z_array_scaled = comm.comm.bcast(Z_array_scaled, root=0)
...
ml_proxy, loop_tracker = create_ml_local_energy_patch(ml_model, y_scaler, node_scaler, edge_scaler, Z_array_scaled, P_hf_ref, E_HF, S_sqrt, h_core, C_a, C_b, aoslices, has_cupy, N_ATOMS)
...
if rank == 0:
    ...
    # Updated node shape size reflection in metric tracking 
    graph_nodes_bytes = afqmc.nwalkers * N_ATOMS * 2 * 4
