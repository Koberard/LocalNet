...
class BroadcastStatic(layers.Layer):
    def call(self, inputs):
        ...
        target_shape = tf.concat([[batch_size], tf.shape(x_static)], axis=0)
        return tf.broadcast_to(expanded, target_shape)
...
class GraphInteraction(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.gate_net = layers.Dense(self.units, activation='sigmoid', kernel_initializer='glorot_uniform')
        self.node_proj = layers.Dense(self.units, kernel_initializer='he_normal')
        super().build(input_shape)

    def call(self, inputs, training=False):
        node_feats, edge_feats, adjacency = inputs
        
        target_shape = tf.concat([tf.shape(edge_feats)[:-1], [tf.shape(node_feats)[-1]]], axis=0)
        h_i = tf.broadcast_to(tf.expand_dims(node_feats, axis=2), target_shape)
        h_j = tf.broadcast_to(tf.expand_dims(node_feats, axis=1), target_shape)

        mlp_input = tf.concat([h_i, h_j, edge_feats], axis=-1)
        messages = self.update_mlp(mlp_input, training=training)
        
        gate = self.gate_net(mlp_input)
        ...
        aggr_messages = tf.reduce_sum(messages, axis=2)
        projected_nodes = self.node_proj(node_feats)
        return projected_nodes + aggr_messages
...
def build_gnn_model(dist_matrix, adj_matrix):
    # UPDATED SHAPES: 12 for nodes, 50 for edges (Complex Flattened)
    input_nodes = layers.Input(shape=(N_ATOMS, 12), name="Node_Features") 
    input_edges_dyn = layers.Input(shape=(N_ATOMS, N_ATOMS, 50), name="Edge_Density_Dyn")
    ...
    for _ in range(2): 
        x = GraphInteraction(64, dropout_rate=0.1)([x, combined_edges, static_adj])
...
def create_ml_local_energy_patch(ml_model, node_scaler, edge_scaler, y_scaler, P_hf_ref, E_hf_ref, S_sqrt, h_core_dyn, C_a, C_b, aoslices, Z_one_hot, use_gpu, n_atoms):
    ...
    def local_energy_single_det_uhf(system, hamiltonian, walkers, trial):
        ...
        # Pull to CPU as numpy for the graph feature extraction loop
        delta_P = cp.asnumpy(P_lowdin - P_hf_ref_xp) if use_gpu else P_lowdin - P_hf_ref_xp
        
        # --- UPDATED: COMPLEX FLATTENED GRAPH FEATURE EXTRACTION ---
        MAX_AOS = 5
        X_nodes_raw = np.zeros((nwalkers, n_atoms, MAX_AOS * 2), dtype=np.float32)
        X_edges_dyn = np.zeros((nwalkers, n_atoms, n_atoms, MAX_AOS * MAX_AOS * 2), dtype=np.float32)

        for i in range(n_atoms):
            p0, p1 = aoslices[i][2], aoslices[i][3]
            n_aos_i = p1 - p0
            
            local_diag = np.diagonal(delta_P[:, p0:p1, p0:p1], axis1=1, axis2=2)
            X_nodes_raw[:, i, :n_aos_i] = local_diag.real
            X_nodes_raw[:, i, MAX_AOS : MAX_AOS + n_aos_i] = local_diag.imag
            ...
            for j in range(n_atoms):
                ...
                block = delta_P[:, p0:p1, q0:q1]
                n_cross = n_aos_i * n_aos_j
                
                X_edges_dyn[:, i, j, :n_cross] = block.real.reshape(nwalkers, -1)
                X_edges_dyn[:, i, j, (MAX_AOS*MAX_AOS) : (MAX_AOS*MAX_AOS) + n_cross] = block.imag.reshape(nwalkers, -1)

        X_nodes_Z = np.tile(Z_one_hot[None, ...], (nwalkers, 1, 1))
        X_nodes = np.concatenate([X_nodes_raw, X_nodes_Z], axis=-1)
        X_edges = X_edges_dyn
        
        # --- NEW: STRICT MASKED SCALING ON THE FLY ---
        padding_mask_nodes = (X_nodes == 0.0)
        B, N, F_n = X_nodes.shape
        X_nodes_scaled = node_scaler.transform(X_nodes.reshape(-1, F_n)).reshape(B, N, F_n)
        X_nodes_scaled[padding_mask_nodes] = 0.0  # Erase the ghost orbitals!
        
        padding_mask_edges = (X_edges == 0.0)
        B, N1, N2, F_e = X_edges.shape
        X_edges_scaled = edge_scaler.transform(X_edges.reshape(-1, F_e)).reshape(B, N1, N2, F_e)
        X_edges_scaled[padding_mask_edges] = 0.0  # Erase the ghost orbitals!
        
        # Inference
        preds_scaled = fast_predict([X_nodes_scaled, X_edges_scaled]).numpy()
        E_corr_delta = y_scaler.inverse_transform(preds_scaled.reshape(-1, 1)).flatten()
...
if rank == 0:
    ...
    Z_one_hot = np.zeros((N_ATOMS, 2), dtype=np.float32)
    for i, z in enumerate(atomic_numbers):
        if z == 1: Z_one_hot[i, 0] = 1.0
        elif z == 6: Z_one_hot[i, 1] = 1.0
    ...
    node_scaler = joblib.load(os.path.join(DEPLOY_DIR, f"node_scaler_{SYSTEM_NAME}.save"))
    edge_scaler = joblib.load(os.path.join(DEPLOY_DIR, f"edge_scaler_{SYSTEM_NAME}.save"))
    ...
    # UPDATED DUMMIES: 12 node features, 50 edge features
    dummy_nodes = np.zeros((1, N_ATOMS, 12), dtype=np.float32)
    dummy_edges = np.zeros((1, N_ATOMS, N_ATOMS, 50), dtype=np.float32)
    ml_model([dummy_nodes, dummy_edges])
    
    ml_model.load_weights(WEIGHTS_PATH)
else: 
    E_HF = C_a = C_b = S_sqrt = h_core = ml_model = y_scaler = node_scaler = edge_scaler = P_hf_ref = aoslices = mol = mf = Z_one_hot = None
...
Z_one_hot = comm.comm.bcast(Z_one_hot, root=0)
...
ml_proxy, loop_tracker = create_ml_local_energy_patch(ml_model, node_scaler, edge_scaler, y_scaler, P_hf_ref, E_HF, S_sqrt, h_core, C_a, C_b, aoslices, Z_one_hot, has_cupy, N_ATOMS)
...
if rank == 0:
    ...
    model_weights_mb = (model_params * 4) / (1024 ** 2)  
    
    # UPDATED MEMORY METRICS (12 nodes, 50 edges)
    graph_nodes_bytes = afqmc.nwalkers * N_ATOMS * 12 * 4
    graph_edges_bytes = afqmc.nwalkers * N_ATOMS * N_ATOMS * 50 * 4
